   ...
   if($passwordConfirm ==''){
        ...
        if($password != $passwordConfirm){
         ...
         if(strlen($password)>20){ // Max 
          ...
          $sql="select * from ph_loginsignup where (username='$username' or email='$email');";
...
$row = mysqli_fetch_assoc($res);
    ...
    $password = $password;
            ...
            $result = mysqli_query($dbc,"INSERT INTO ph_loginsignup (Pharmacy_Name,Pharmacy_Address,username, email, password, date)
             values('$Pharmacy_Name','$Pharmacy_Address','$username','$email','$password','$date')");
...
?>
  ...
  <div class="form-group">
        ...
        <label class="label_txt">Pharmacy_Name</label>
    <input type="text" class="form-control" name="Pharmacy_Name" value="<?php if(isset($error)){ echo $_POST['Pharmacy_Name'];}?>" required="">
  </div>
  <div class="form-group">
    <label class="label_txt">Pharmacy_Address</label>
    <input type="text" class="form-control" name="Pharmacy_Address" value="<?php if(isset($error)){ echo $_POST['Pharmacy_Address'];}?>" required="">
  </div>
...
<div class="form-group">
    <label class="label_txt">Pharmacy_Email </label>
